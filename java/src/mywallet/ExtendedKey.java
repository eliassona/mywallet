...
public class ExtendedKey {
	public static final long HARDENED = 0x80000000;
     ...
     */
    public ExtendedKey(byte[] keyHash, boolean compressed, long sequence, int depth, int parentFingerprint, ECKey ecKey) {
    ...
    private ExtendedKey getChild(long i) throws Exception {
        ...
        int startIx = (i & HARDENED) == HARDENED ? 1: 0;
        byte[] child = new byte[pub.length + 4 + startIx];
        System.arraycopy(pub, 0, child, startIx, pub.length);
        //now some byte shifting
        child[startIx + pub.length] = (byte) ((i >>> 24) & 0xff);
        child[startIx + pub.length + 1] = (byte) ((i >>> 16) & 0xff);
        child[startIx + pub.length + 2] = (byte) ((i >>> 8) & 0xff);
        child[startIx + pub.length + 3] = (byte) (i & 0xff);
...
//    }
    ...
    private class ExtendedKeySerializer {
        ...
        public String serialize(byte[] version,
            ...
            out.write((int) ((sequence >>> 24) & 0xff));
            out.write((int) ((sequence >>> 16) & 0xff));
            out.write((int) ((sequence >>> 8) & 0xff));
            out.write((int) (sequence & 0xff));
            out.write(chainCode);
            if (version.equals(xprv)) {
                out.write(0x00);
            }
            out.write(keyBytes);
            return ByteUtil.toBase58WithChecksum(p(out.toByteArray()));
        }

		private byte[] p(byte[] byteArray) {
    ...
    public static void main(String[] args) throws Exception {
    	List<Integer> l = java.util.Arrays.asList(-67, 22, -66, -27, 57, 97, -92, 125, 106, -40, -120, -30, -107, 69, 67, 74, -119, -67, -2, -107);
        int fingerprint = 0;
        for (int i = 0; i < 4; i++) {
            fingerprint <<= 8;
            fingerprint |= l.get(i) & 0xff;
        }
        System.out.println("fp: " + fingerprint);
	}
}